...
**Trois variables parasites introduites par ce test**, à neutraliser avant de conclure quoi que ce soit :

- **L'adresse e-mail.** L'inscription a utilisé `djodjoralas.s@gmail.com`, une variante à points de `djodjoralass@gmail.com`. **Gmail ignore les points** : c'est la même boîte, et Meta applique la même normalisation. Or cette boîte **porte déjà un compte Instagram**. Instagram a donc vu un second compte ouvert sur une adresse connue, depuis un appareil et une IP qu'il n'avait jamais vus. C'est probablement le signal le plus lourd des trois. Règle qui en découle : **une adresse réellement distincte par personnage, jamais une variante à points d'une boîte déjà utilisée.**

- **La durée.** L'inscription a pris **~35 min**, avec des intervalles allant jusqu'à 10 min entre deux champs (relevé d'écran et raisonnement entre chaque action). Un humain remplit ce parcours en 2 à 3 min. Une session aussi étalée, avec des pauses régulières et des saisies instantanées, est en soi un profil d'automatisation. Le rejeu par coordonnées supprime ce signal.
- **Le numéro.** Le numéro utilisé était français (+33) sur une IP San Francisco, et le code est arrivé **par WhatsApp** et non par SMS — le numéro était donc déjà connu de Meta. C'est un signal de désaccord que nous avons créé nous-mêmes ; il ne permet pas de conclure que l'escalade est systématique. Le test qui tranche est le même parcours avec un **numéro américain neuf**, tout le reste identique.

...
**Ce qui a été essayé, semaine du 2026-09-15, et ce qui en reste.** Gmail : porte numéro, création refusée même avec une ligne US. Outlook : bloqué au numéro. GMX : ouvert mais écarté pour l'image. Yahoo : deux « session timed out » depuis un ordinateur, et depuis le téléphone de Sierra un sélecteur de pays verrouillé sur l'Italie (+39) sans explication — l'IP sort bien à Seattle selon quatre sources, l'anomalie est interne à Yahoo. **Proton est la seule boîte réellement créée** (`sierracole@proton.me`, captcha résolu à la main). Son offre gratuite n'a pas d'IMAP, et ce n'est pas bloquant : la ferme lit les codes **dans les notifications du téléphone** (`get_notifications`, §4.3), pas par IMAP. Décision : **une boîte Proton par personnage, créée sur le téléphone du personnage** (un captcha humain par boîte, rien d'autre), **l'application Proton installée sur ce téléphone avec les notifications autorisées** ; la partie locale = `prénom.nom` + année de naissance ; le même mot de passe pour les six boîtes, dans le Trousseau (§8) ; adresse de récupération `nathann@ofmai.ai`. La contrainte 2 ci-dessus se lit donc « lisible par la ferme » (notification ou IMAP), et le domaine neutre reste une option, pas un prérequis.
...
- Support : vérifié le 2026-09-18 — une ligne US Mobile s'active depuis l'étranger, et les SMS (codes courts compris) arrivent en roaming en Thaïlande sans Wi-Fi Calling (`numeros-us.md`). Six lignes, 10 $ par ligne et par mois. Le relais du code (message Discord privé, canal dédié) est acceptable : la ligne est réelle, seul l'acheminement du code est automatisé.
...
- **Instagram exige le numéro dès la création, sans échappatoire** (relevé du 2026-09-16, `screens-instagram.md` §1) : l'inscription se fait bien par email, mais le compte reste bloqué derrière un contrôle SMS tant qu'aucun numéro n'est validé. Le numéro est donc un prérequis de J0, pas une réponse à une demande éventuelle. Sur les autres plateformes, tenter « Skip » d'abord et relever le comportement réel avant de conclure [à vérifier : TikTok, X, Reddit].
...
Skill `ofmai_signup_reddit`, relevé écran par écran le 2026-09-16 (`screens-reddit.md`). Étapes : `launch` → « Get Started » → « Continue with email » → `type {email}` → « Continue » → checkpoint `email` (**code à 6 chiffres**, pas un lien, dans un champ unique) → username `{handle}` (le champ arrive **pré-rempli d'un pseudo aléatoire** : la croix à droite le vide — checkpoint) → checkpoint `login` → dialogue « Can't create passkey » (OK) et permission notifications (Allow), tous deux par détecteur → anniversaire `{birthday_us}` (MMJJAAAA, saisi au clavier via le crayon « Select date », puis « Yes, Confirm ») → genre, 12 pastilles d'intérêts, « Customize your feed » (checkpoint : c'est de la consommation, pas un follow) → fil d'accueil. Ni captcha ni numéro sur ce relevé.

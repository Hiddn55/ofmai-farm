...
```
génération ──▶ banque ──▶ file ──▶ ferme ──▶ post ──▶ métriques ──▶ OFMAI (attribution, rapport)
 OFMAI         OFMAI      OFMAI    Mac mini  téléphone  téléphone/    prod + PostHog + Discord
 (routes,      (QA, re-   (Social- (bridge,  ou API     API/observ.
  webhooks)    rendu, S3)  Publica- planner,
                           tion)    scheduler)
```

1. **Génération** (OFMAI). Le workflow quotidien choisit les prompts (Foxy, radar, hooks, presets) et appelle les routes admin : `POST /api/admin/generate-batch` (images, Soul ou LoRA) et `POST /api/hot-sfw/generate` (clips). Chaque route crée une `Generation` en `processing`, dispatche, répond ; le webhook du moteur écrit la clé S3 et passe en `completed` (invariant 1 de `CLAUDE.md`). Détail : `content-pipeline.md` §4.
2. **Banque** (OFMAI). QA « 3 pour 1 » (refus moteur, doublon `phash`, identité Rekognition, anatomie) → `ContentAsset.status = kept` → re-rendu par plateforme + filigrane → `ContentAssetVariant` sous `social/<characterId>/variants/<platform>/` → légende par persona → conformité → variante `ready` → `SocialPublication` en `queued` avec `scheduledAt`. Détail : `content-pipeline.md` §5-8.
3. **File** (OFMAI → Mac mini). Le daemon `bridge` fait `GET /api/farm/queue?platform=&handle=&limit=3` avec `x-farm-secret`, réclame chaque item (`POST /api/farm/queue/{id}/claim`, bail 60 min), télécharge l'URL signée (`signUrlSafe`, TTL `S3_SIGNED_URL_TTL`, défaut `10800` s), vérifie `sha256`, pousse le fichier sur le téléphone en ADB, statut `staged`. Détail : `bridge-ofmai-farm.md` §3 et §6.
4. **Ferme** (Mac mini → téléphone). Le planner (`python -m gitd.farm.cli daemon`) transforme les slots du jour en jobs `skill_workflow` `warm_session` (priorité 2, `trigger="farm"`, `max_duration_s = (minutes + 10) × 60`) ; le bridge enfile un job `post_video` / `post_photo` / `post_story` selon `SocialPublication.format` à `scheduled_at` ± 20 min (`LATE_TOLERANCE_MINUTES`), et les `metrics_pull` à 24 / 72 / 168 h (`bridge-ofmai-farm.md` §6). Le scheduler Ghost (thread, tick 30 s, un job actif par téléphone) lance `gitd/skills/_run_skill.py` en sous-processus ; la ligne `Data: {…}` du log est parsée et servie par `GET /api/scheduler/history/{run_id}/result`.
5. **Post**. Canal `device` : Instagram toujours, TikTok pendant la chauffe (workflow `post_video`, média « le plus récent de la galerie », toggle AIGC best effort). Canal `api` : TikTok en `api_mode` par le connecteur MCP, X et Reddit par leurs API depuis le Mac mini à travers l'IP statique du personnage. Arbitrage app/API par geste : `publishing.md`.
6. **Retour** (Mac mini → OFMAI). Tout événement est d'abord écrit dans `farm_outbox` (même commit que la ligne de ledger), puis envoyé par lots à `POST /api/farm/events` : `posted`, `post_failed`, `health_signal`, `session_summary`, `metrics`. `event_id` unique = idempotence. Détail : `bridge-ofmai-farm.md` §4.
7. **Métriques**. Workflow `metrics_pull` à 24 / 72 / 168 h (écran du téléphone, `publish-api.ts --metrics` pour X/Reddit, ou profil observateur pour les canaris de shadowban) → `farm_post_metrics` → événement `metrics` → `SocialPostMetric`. Détail : `metrics-attribution.md` §5, `health-canaries.md`.
8. **OFMAI ferme la boucle**. Les visiteurs arrivent avec `utm_source` × `utm_content=<personnage>` (schéma tranché dans `metrics-attribution.md` §1 : `instagram`, jamais `ig-organic`) ; `POST /api/attribution` écrit `User.utmSource/utmContent` une seule fois ; la vérité des payants est `User` × `Subscription` lue par le skill `prod` ; PostHog explique les écarts ; la phase `Rapport` du workflow poste un message Discord par jour ; un `health_signal` reçu déclenche `sendDiscordAlert` dans la minute (`rules.md` R33).
...
| S3 et Rekognition | `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `S3_BUCKET_NAME` (`hiddn2`), `S3_PUBLIC_BUCKET` (`hiddn2-public`), `S3_REGION`, `S3_SIGNED_URL_TTL`, `CDN_PUBLIC_URL` | env prod OFMAI | `lib/storage/s3.ts`, `lib/kyc/rekognition.ts` |
| Moteurs | `HIGGSFIELD_API_KEY`, `HIGGSFIELD_API_SECRET`, `HIGGSFIELD_API_HOST` (Soul) ; `WAVESPEED_API_KEY` (clips) ; `XAI_API_KEY` (Grok : prompts, légendes, conformité) | env prod OFMAI | `lib/providers/*` |
...
| Proxies IPRoyal, profils téléphone | identifiants proxy `HOST:PORT:USER:PASS` par profil | console GeeLark + Trousseau `ofmai-proxy-<slug>-static` / `-mobile` (lus par le publieur X/Reddit) | GeeLark, `scripts/social/publish-api.ts` ; jamais dans `farm_accounts` ni dans un `config_json` de job (`rules.md` R10) |
...
| Proxy statique en panne | l'appareil n'a plus de réseau ; ne **jamais** remplacer l'IP (`rules.md` R19) | `accounts disable` le temps du ticket IPRoyal ; `enable` au retour de la même IP |
| Bascule mobile pendant une action | motif « suspicious login » | interdit par construction : la bascule n'a lieu que dans le creux ≥ 45 min entre deux sessions (`plan_sessions`, `rules.md` R20) |
| Checkpoint humain non résolu | run en `awaiting_human`, `DEFAULT_TIMEOUT_S = 600` → `timed_out` (relançable) ; `0` = attente infinie | un humain fournit le code sur l'appareil puis `POST /api/skills/runs/{id}/resume` ; alerte Discord dans la minute (R33) |
| Signal santé sur un compte | session arrêtée sans réessai ; `action_blocked` → `COOLDOWN` 48 h (`COOLDOWN_HOURS`) et une phase en arrière ; `shadowban` → 7 j (`SHADOWBAN_DAYS`) ; `suspended` → quarantaine 30 j (`QUARANTINE_DAYS`) ; `verification` / `logged_out` / `suspended` → un humain | `health-canaries.md` ; `accounts clear-health` après avoir regardé l'écran (R27) |
...
|---|---|---|---|
| Nathan (Bangkok, UTC+7) | fondateur | décisions J4/J7/J14, achats (GeeLark, IPRoyal, SIM), validation des subs et des textes de DM ; checkpoints et `clear-health` sur la plage **18:00-00:00 ET** (05:00-11:00 Bangkok) | — |
| Assistant Paris | personne nommée (SSH + Discord + SIM physique) | checkpoints, SMS, `clear-health --reason`, `resume`, canaris à regarder sur la plage **08:00-17:00 ET** (14:00-23:00 Paris) | décisions J4/J7/J14, `platform resume` après S4 |
| Agents (workflows §12 de `build-plan.md`) | launchd | tout le reste | `clear-health`, `resume`, `platform resume`, `accounts api-mode` (R27) |
...
Hebdomadaire (lundi, Paris) : DM X/Reddit lus sur l'appareil (`publishing.md` §9) ; stock de variantes `ready` ≥ 3 × cap par plateforme (`content-pipeline.md` §9) ; `tested_on` re-rempli après toute mise à jour d'app (R35) ; sauvegarde de la veille présente sur S3. Mensuel : test de restauration (§7).

Décisions J4/J7/J14 : lues par Nathan sur le rapport du jour J (`metrics-attribution.md` §7), écrites dans `docs/social/decisions/AAAA-MM-<plateforme>-<decision>.md` (dossier à créer à la première décision), listées dans `INDEX.md`.

...
> **Répond à** : comment la ferme (Mac mini, Ghost + `gitd/farm/`) reçoit le contenu, les légendes, les commentaires et les fiches persona d'OFMAI, et comment OFMAI reçoit en retour les posts, les métriques, les signaux santé et les résumés de session
...
model SocialAccount {
  ...
  handle        String
  role          String   @default("persona") // persona | brand | observer

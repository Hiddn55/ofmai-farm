...
### 3.7 `GET /api/farm/targets?character_id=cmf…&platform=instagram&n=5`

Les comptes que la chauffe visite et, si le ledger l'autorise, suit — tirés du **radar**, pour la niche du personnage (décision du 2026-09-15, `warming-policy.md` §8.2). Même forme que §3.4 : la ferme demande, OFMAI choisit, la ferme rend compte de ce qu'elle a consommé.

Paramètres : `character_id`, `platform`, `n` (défaut 5, maximum 10 — le plafond `searches` en cruise est de 5 par jour).

Sélection côté OFMAI : `TrackedInfluencer` où `niche` = `niche.radar` de la fiche persona (`personas.md` §1, `lib/social/personas.ts`), `market = "US"`, `status = "active"`, `enabled = true` ; on exclut tout `handle` déjà servi à ce compte depuis moins de 90 jours (`SocialTargetUse`, §5.1) ; à rang égal, `accountType = "reelle"` d'abord (l'inverse de la sélection de contenu, qui préfère `"ia"` — `content-pipeline.md` §4.1) ; ordre déterministe par `(socialAccountId, date)`, comme le budget du jour, pour que deux appels le même jour rendent la même liste. Chaque ligne rendue est réservée `now + 24 h`.

```json
{"targets":[{"id":"tgt_7","handle":"lena.trains","display_name":"Lena","niche":"fitness",
  "followers":48210,"account_type":"reelle","search_query":"lena.trains",
  "reserved_until":"2026-09-16T10:12:00-07:00"}]}
```

- `handle` est le pseudo nu, minuscule, sans `@` (`TrackedInfluencer.handle`). `search_query` est ce que la session tape dans la recherche de l'application : le pseudo tel quel aujourd'hui, champ distinct pour laisser la place à une variante par plateforme.
- **Le radar n'indexe qu'Instagram** (`TrackedInfluencer.platform`, défaut `instagram`). Sur `tiktok` et `x`, la cible reste une requête de recherche et la session retombe sur le hashtag de niche si l'application ne rend rien [à vérifier : recouvrement des pseudos entre plateformes]. Sur `reddit`, la route rend **toujours** `{"targets":[]}` : il n'y a pas d'influenceuse à suivre, le détour `r/<niche>` reste la règle (`warming-policy.md` §7).
- Pool épuisé pour ce compte : liste vide, jamais une erreur ; la session retombe sur le hashtag, c'est-à-dire sur le comportement d'aujourd'hui.
...
- Consommation remontée dans `session_summary` : `targets_used` (§4.1). Une cible non consommée redevient disponible à l'expiration de la réservation, comme un commentaire.
...
| `farm_target_cache` | `target_id` (OFMAI), `account_id`, `handle`, `search_query`, `fetched_at`, `used_at` NULL — même mécanique que `farm_comment_cache` pour les cibles du radar (§3.7) ; `planner.job_config` en tire `params.targets` (`\n`-séparés) comme il tire `params.comments` |
...
6. Rafraîchit `farm_comment_cache` pour chaque compte sous 5 commentaires disponibles (`GET /api/farm/comments`, `n=10`, par `kind`) et `farm_target_cache` pour chaque compte sous 5 cibles disponibles (`GET /api/farm/targets`, `n=5`, §3.7) ; une liste vide n'est pas une erreur (pool de niche épuisé, ou plateforme `reddit`).
...
Le planner passe `params.comments` depuis `farm_comment_cache` et `params.targets` depuis `farm_target_cache` (`\n`-séparés dans les deux cas) ; `WarmSessionAction` ajoute `session_id`, `comments_used` et `targets_used` au `Data:` et écrit le `session_summary` dans `farm_outbox` en fin d'`execute` (même transaction que la dernière écriture ledger).

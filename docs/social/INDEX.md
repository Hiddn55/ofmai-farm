...
| `numeros-us.md` | **le goulot d'approvisionnement est levé** : US Mobile s'active depuis l'étranger (ligne 415 activée depuis la Thaïlande), les SMS entrants — **y compris depuis un numéro court** — arrivent en itinérance sans module de roaming ni appels Wi-Fi, et TikTok a accepté le numéro. 10 $/ligne/mois, soit 180 $ pour six lignes sur trois mois | **vérifié de bout en bout le 2026-09-18** |
| `screens-tiktok-actions.md` | actions TikTok exercées sur un vrai compte : like, favori, commentaire, réponse, publication, recherche et profil vérifiés **par incrément de compteur** ; ids obfusqués donc sélection par `content-desc` ; **`uiautomator` échoue dès qu'une vidéo joue** (fil et résultats de recherche) ; et un **faux positif documenté** — la disparition du bouton Suivre ne prouve pas le suivi | **vérifié sur téléphone le 2026-09-18** |
| `warming-policy.md` | plafonds, rythmes, sessions, chauffe orientée et sa mesure (score Explore, `explore_score.py`), pools de commentaires, cibles de suivi tirées du radar par niche, santé et `api_mode` d'un compte — miroir de `policy.py` | en vigueur (§1-§7 bis, §9) |
...
| `runbook-lancement.md` | Lancer les chauffes sur le Mac mini : les deux processus, la préparation de la machine, `accounts add`, le test immédiat, comment savoir que ça tourne | avant le premier lancement, et à chaque nouveau compte |
